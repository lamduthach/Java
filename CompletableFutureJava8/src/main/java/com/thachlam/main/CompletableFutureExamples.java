...
public class CompletableFutureExamples {
    ...
    public static void main(String[] args) {
        ...
        CompletableFuture.runAsync(() -> {
            for (int i = 0; i < 10; i++) {
                try {
                    Thread.sleep(5000);
                    throw new RuntimeException(new CustomException("test"));
                } catch (InterruptedException ex) {
                    System.out.println(ex.getMessage());
                }
                System.out.println("CompletableFuture.runAsync");
            }
        }, executorService).whenComplete((t, ex) -> {
            if (ex != null) {
    ...
    static void thenApplyAsyncWithExecutorExample() {
        ...
        CompletableFuture<String> cf = CompletableFuture.completedFuture("message").thenApplyAsync(s -> {
            ...
            randomSleep();
            return s.toUpperCase();
        }, executor).whenComplete((t, u) -> executor.shutdown());
    ...
    static void acceptEitherExample() {
        ...
        CompletableFuture<Void> cf = CompletableFuture.completedFuture(original)
                ...
                .acceptEither(CompletableFuture.completedFuture(original).thenApplyAsync(s -> delayedLowerCase(s)), s -> result.append(s).append("acceptEither"));
        cf.join();
